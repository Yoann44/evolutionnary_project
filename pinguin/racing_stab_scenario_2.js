{
	...
	this.minFitnessTheta = 1.0;
	this.minFitnessPhi = 1.0;
	this.minFitnessPsi = 1.0;
    ...
    afterSimulationStep: function() {
		...
		if(this.minFitnessTheta > (1 - 2 * Math.abs(diffTheta/3.14159)) {
			this.minFitnessTheta = (1 - 2 * Math.abs(diffTheta/3.14159);
		}

		if(this.minFitnessPhi > (1 - 2 * Math.abs(diffPhi/3.14159)) {
			this.minFitnessPhi = (1 - 2 * Math.abs(diffPhi/3.14159);
		}

		if(this.minFitnessPsi > (1 - 2 * Math.abs(diffPsi/3.14159)) {
			this.minFitnessPsi = (1 - 2 * Math.abs(diffPsi/3.14159);
		}
    ...
    endSimulation: function() {
		...
		console.log("Min angle fitness - Theta:" + this.minFitnessTheta + ", Phi:" + this.minFitnessPhi + ", Psi:" + this.minFitnessPsi);
		...
		var min_angle_fit = this.minFitnessTheta * this.minFitnessPsi;

		if(min_angle_fit > 0.75) {
			min_angle_fit = -3.0 * (1.0 - min_angle_fit) + 1.0;
		}
		else {
			min_angle_fit = -1.0/3.0 * (1.0 - min_angle_fit) + 1.0/3.0;
		}
